...
class SubDataset:
    def __init__(self, sub_meta, cl, transform=transforms.ToTensor(), target_transform=identity):
        ...
        self.transform_aug =TransformLoader(224, jitter_param=jitter_param).get_composed_transform(aug=True)

    def __getitem__(self, i):
        img = self.transform(self.sub_meta[i].copy())
        img_aug = self.sub_meta[i]
...
class SetDataManager(DataManager):
    def __init__(self, image_size: object, n_way: object = 5, n_support: object = 5, n_query: object = 16, n_eposide: object = 100) -> object:
    ...
    def get_data_loader(self, aug): #parameters that would change on train/val set
        ...
        data_loader_params = dict(batch_sampler = sampler,  num_workers = 12, pin_memory = True, collate_fn=list_set_collate)
        data_loader = torch.utils.data.DataLoader(dataset, **data_loader_params)
        return data_loader
